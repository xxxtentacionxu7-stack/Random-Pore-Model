...
%% PlotPulseValidation - Measured vs predicted pulse validation figures
    ...
    subplot(4,1,[1 2 3]);
    hold on;
    plot(comp.t_meas, comp.X_meas, '-', 'Color', style.colors.blue, ...
        'LineWidth', style.lineWidth, 'DisplayName', 'Measured');
    plot(comp.t_pred, comp.X_pred, '--', 'Color', style.colors.red, ...
        'LineWidth', style.lineWidth, 'DisplayName', sprintf('Predicted (RPM+SF=%.3f)', comp.SF_used));
    ...
    for c = 0:n_cycles-1
        ...
        if x_start < comp.t_pred(end)
            patch([x_start x_end x_end x_start], [0 0 1.05 1.05], ...
                style.colors.lightBlue, 'EdgeColor', 'none', 'FaceAlpha', 0.25, ...
                'HandleVisibility', 'off');
        end
    end

    ylabel('Conversion X', 'FontName', style.fontName, 'FontSize', style.labelFontSize);
    legend('Location', 'southeast', 'FontName', style.fontName, 'FontSize', style.legendFontSize, 'Box', 'off');
    ylim([0, 1.05]);

    subplot(4,1,4);
    stairs(comp.t_pred, comp.valve, '-', 'Color', style.colors.red, 'LineWidth', style.thinLineWidth);
    ylim([-0.2, 1.2]);
    yticks([0 1]);
    yticklabels({'Off', 'On'});
    ...
    plot(comp.t_meas, resid, '-', 'Color', style.colors.black, 'LineWidth', style.thinLineWidth);
    hold on;
    yline(0, '--', 'Color', style.colors.red, 'LineWidth', 1);
    xlabel('Time (min)', 'FontName', style.fontName, 'FontSize', style.labelFontSize);
    ylabel('Residual (measured - predicted)', 'FontName', style.fontName, 'FontSize', style.labelFontSize);
    ApplyFigureFormat(gcf);
