...
%% PlotSynergyAndPulse - Synergy and pulse prediction figures
    ...
    for i = 1:n
        s = syn_all{i};
        plot(s.X, s.SF, '-', 'Color', colors(i,:), 'LineWidth', style.lineWidth, ...
            'DisplayName', sprintf('%s  (SF_{avg}=%.2f)', s.label, s.SF_mean));
    end
    yline(1, '--', 'Color', style.colors.gray, 'LineWidth', style.thinLineWidth, ...
        'DisplayName', 'Baseline SF = 1');
    xlabel('Conversion X', 'FontName', style.fontName, 'FontSize', style.labelFontSize);
    ylabel('Synergy factor  SF = rate_{mix}/rate_{linear}', 'FontName', style.fontName, 'FontSize', style.labelFontSize-1);
    ...
    if save_fig; ExportFigure(gcf, 'Fig_SynergyFactor'); end

    figure('Name', 'Rate Comparison', 'Color', style.figureColor, 'Position', style.figSizeMid);
    hold on;
    for i = 1:n
        s = syn_all{i};
        plot(s.X, s.rate_mix, '-', 'Color', colors(i,:), 'LineWidth', style.lineWidth, ...
            'DisplayName', sprintf('%s measured', s.label));
        plot(s.X, s.rate_linear, '--', 'Color', colors(i,:), 'LineWidth', style.thinLineWidth, ...
    ...
    legend('Location', 'best', 'FontName', style.fontName, 'FontSize', style.legendFontSize, 'Box', 'off');
    ApplyFigureFormat(gcf);
    if save_fig; ExportFigure(gcf, 'Fig_RateComparison'); end
    ...
    if ~isempty(pulse)
        ...
        for c = 0:n_cycles-1
            ...
            if x_start < pulse.t(end)
                patch([x_start x_end x_end x_start], [0 0 1.05 1.05], ...
                    style.colors.lightBlue, 'EdgeColor', 'none', 'FaceAlpha', 0.3, ...
                    'HandleVisibility', 'off');
            end
        end
        legend({'Predicted X'}, 'Location', 'southeast', 'FontName', style.fontName, 'FontSize', style.legendFontSize, 'Box', 'off');

        subplot(3,1,3);
        stairs(pulse.t, pulse.valve, '-', 'Color', style.colors.red, 'LineWidth', style.thinLineWidth);
        ylim([-0.2, 1.2]);
        yticks([0 1]);
        yticklabels({'Off', 'On'});
        ...
        ApplyFigureFormat(gcf);
        if save_fig; ExportFigure(gcf, 'Fig_PulsePrediction'); end
        ...
        ylim([0, 1.05]);
        ApplyFigureFormat(gcf);
        if save_fig; ExportFigure(gcf, 'Fig_PulseFinal'); end
    end
end
